...
## args for elr loss
parser.add_argument('--l', type=float, default=3,
    help='regularization strength')
parser.add_argument('--beta', type=float, default=0.7,
    help='temporal ensembling momentum for target estimation')
...
def train(args, mymodel, optimizer, train_data, valid_data=None, test_data=None):
    ...
    loss_func = elr_loss(len(train_data.dataset), args.num_class, args.l, args.beta)
    ...
    for epoch in range(1, EPOCH + 1):
        ...
        for i, data in enumerate(train_data):
            ...
            input_ids, attention_mask, labels, index = [Variable(elem.cuda()) for elem in data]
            optimizer.zero_grad()

            logits = mymodel(input_ids, attention_mask, is_training=True)

            out = F.log_softmax(logits)
            loss = loss_func(index, logits, labels)
